...
class MinimaxAI(BaseAI):
    ...
    def get_valid_moves_generator(self, board, player):
        """Generator for yielding valid moves for a given player."""
        for pos in board:
            if board[pos] == player:
                # Assuming `self.moves.get_valid_moves()` returns a list of valid moves for that piece
                for move in self.moves.get_valid_moves(pos[0], pos[1]):
                    yield pos, move  # Yield each valid move one by one
    ...
    def better_evaluate(self, board, player):
        ...
        nplayer_moves = 0
        for _, _ in self.get_valid_moves_generator(board, player):  # Use the generator
            nplayer_moves += 1
